...
class _DiffieHellman {
  ...
  Future<AuthorizationKey> _createAuthKey(
    ...
    if (!_sameBytes(answerHash, hash)) {
      throw Exception('Server DH answer hash mismatch.');
    }
    ...
    if (result is DhGenOk &&
        '0x${_hex(expectedNewNonceNHash.skip(4))}'.toLowerCase() !=
            result.newNonceHash1.toString().toLowerCase()) {
      throw Exception('DH nonce hash mismatch.');
    }
...
bool _sameBytes(Iterable<int> left, Iterable<int> right) {
